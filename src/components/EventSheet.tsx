...
import { ArrowRight, Clock, ExternalLink, MapPin, Navigation, X } from 'lucide-react';
...
export function EventSheet({ event, onClose, onOpenDetail, onNavigate }: Props) {
  ...
  return <section ref={dialogRef} className="event-sheet event-preview-sheet" role="dialog" aria-modal="true" aria-label="地図のイベント概要">
    ...
    {present(event.imageUrl) && !imageFailed && <figure className="event-sheet-media"><img className="event-sheet-image" src={event.imageUrl} alt="" referrerPolicy="no-referrer" onError={() => setImageFailed(true)} />{present(event.imageSourceUrl || event.officialUrl || event.sourceUrl) && <figcaption><a href={event.imageSourceUrl || event.officialUrl || event.sourceUrl} target="_blank" rel="noreferrer">公式画像・出典 <ExternalLink size={11} aria-hidden="true" /></a></figcaption>}</figure>}
    <div className="event-sheet-body">
      ...
      <div className="event-preview-actions">
        ...
        <button type="button" disabled={!canNavigate} onClick={() => maps('apple')}>{canNavigate ? '経路を見る' : '経路案内なし'}</button>
      </div>
      {!canNavigate && <p className="event-navigation-note">住所・会場情報が未確認です。詳細ページから公式情報をご確認ください。</p>}
    </div>
  </section>;
}
